...
class VideoThread(QThread):
    ...
    scanStampProcess  = Signal()
    dataStamp = Signal(dict)
    signalPLC = Signal(str)
    ...
    def __init__(self):
        ...
        self.cap.set(3,2048)
        self.cap.set(4,2048)
        self.scanStampProcess.connect(self.scanProcess)
    def run(self):
        while True:
    ...
    def scanProcess(self):
        ...
        for i in range(10):
                print("Run number: ", i)
                data = self.findOppositePoint(self.cv_img,160,100000)
                if data is not None:
                    return self.dataStamp.emit(data)
        return self.dataStamp.emit(None)
    ...
    def findOppositePoint(self,image,val_thresh,areaTem = 10000):
        ...
        if  Point_Cirle is not None and pointCenterTem is not None and dataResult is not None:
                ...
                return {'data': dataResult['data'] , 'end_point': end_point}
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.camera.dataStamp.connect(self.getDataStamp)
        self.camera.signalPLC.connect(self.signalPLCProcess)
        self.oldDataResult = []
        self.dataCode = []
        self.counter = 0
        self.listBarcode  = []
    ...
    def read_barcode_zxing(self, frame):
        ...
        for result in resultScanCode:
            if str(result.format) == "BarcodeFormat.DataMatrix" or str(result.format) == "BarcodeFormat.QRCode":
                ...
                [x4,y4] = result.position.top_left.x,result.position.top_left.y
                return {'data' :result.text.split(","),'pos1': [x1,y1],'pos2': [x2,y2],'pos3': [x3,y3],'pos4': [x4,y4]}
    def signalPLCProcess(self,signal):
        self.sendDataSerial(signal)
        
        pass
    def getDataStamp (self,stampData):
        if stampData is not None:
            self.processData(stampData['data'])
            self.sendDataSerial("OK")
        else:
            self.sendDataSerial("NG")
